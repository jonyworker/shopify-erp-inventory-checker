  ...
  const results = promotionRows.map(item => {
    ...
    if (handles.length > 1) {
      const products = handles.map(handle =>
          indexes.productMetaByHandle.get(handle)
      )
    ...
    const variantRow =
        matches.find(match => match.handle === handle)?.row || {}
    ...
    return {
      ...
      shopifyTitle:
          meta.title ||
          normalizeText(variantRow.Title),

      shopifyPrice:
          normalizePrice(
              variantRow['Variant Price']
          ),

      compareAtPrice:
          normalizePrice(
              variantRow['Variant Compare At Price']
          ),
