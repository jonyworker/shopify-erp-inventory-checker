...
function handleExport() {
  ...
  if (exportType.value === 'csv') {
    downloadCsv('price-compare-result.csv', filteredResults.value)
    return
  }

  downloadExcel('price-compare-result.xlsx', filteredResults.value)
}
...
<template>
  <div>
        ...
        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p class="mt-3 max-w-3xl text-slate-600">
              匯入 PTS TW RRP 多工作表價目表與 ERP 價格檔，選擇品項編碼與價格欄位，系統會自動比對 ERP 出庫單價是否與官方價目表一致。
            </p>
          </div>
        </div>
      </header>
      ...
      <section class="grid gap-4 lg:grid-cols-2">
        ...
        <FileUploadCard
          title="ERP 價格檔"
          description="請上傳 ERP 匯出的價格 Excel，預設比對品項編碼與出庫單價。"
